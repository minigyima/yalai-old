...
installer() {
 ...
 sudo ./yalai/yalai.sh
choice=$(zenity --list --title="Installation finished!" --radiolist --text "YaLAI has finished installing Arch Linux on your system. What would you like to do now?"  --column "Select" --column "Option" FALSE Restart FALSE Close FALSE "Chroot into new system" FALSE "Open pacman.conf")
	if [ "$choice" = "Restart" ]
	then reboot
